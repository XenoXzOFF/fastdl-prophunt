...
AWarn.SelectedLanguage = AWarn.DefaultLanguage
...
hook.Add( "InitPostEntity", "awarn3_loadlanguage", function()
	AWarn.SelectedLanguage = LocalPlayer():GetPData( "awarn3_lang", AWarn.DefaultLanguage )
...
function PANEL:DrawElements()
  ...
  if AWarn:CheckPermission( LocalPlayer(), "awarn_remove" ) then
...
function PANEL:DrawElements( pl )
  if not AWarn:CheckPermission( LocalPlayer(), "awarn_remove" ) then return end
	...
	deleteButton.OnSelected = function()
    if AWarn.Options.awarn_removewhendeletewarning and AWarn.Options.awarn_removewhendeletewarning.value then
...
function PANEL:OnMousePressed( key )
    ...
    if IsValid(AWarn.menu.adminWarningsView.subWarningButton) then AWarn.menu.adminWarningsView.subWarningButton:SetVisible( true ) end
...
function AWarn:UnselectPlayer()
	...
	AWarn.menu.adminWarningsView.selectedplayerid = nil
	 if IsValid(AWarn.menu.adminWarningsView.subWarningButton) then AWarn.menu.adminWarningsView.subWarningButton:SetVisible( false ) end
